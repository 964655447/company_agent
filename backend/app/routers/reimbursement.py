...
async def submit(
    ...
    rec = Reimbursement(employee_id=user.id, category=category.strip(),
...
@router.get("/my")
def my_reimbursements(user: Employee = Depends(get_current_user),
                       db: Session = Depends(get_db)):
...
def review(ticket_id: int, body: ReviewIn,
    ...
    rec.approver_id = manager.id
    db.commit()
    return {"ticket_id": rec.id, "status": rec.status,
            "approver": manager.name, "message": "已通过" if body.action == "approve" else "已驳回"}


@router.get("/report")
async def reimbursement_report(manager: Employee = Depends(require_manager),
                               db: Session = Depends(get_db)):
    perms = set(manager.permission_list)
    emps = db.scalars(select(Employee)).all()
    visible_ids = {e.id: e for e in emps if not perms or e.position in perms or e.id == manager.id}
    if len(perms) >= 5:  # 总经办看全部
