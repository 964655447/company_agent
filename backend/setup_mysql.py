...
def run_salary_sql(conn):
    """建 employee_salary_last_month / employee_salary_this_month 两张表，并灌入上月 26 行真实工资。

    幂等：
      - CREATE TABLE 带 IF NOT EXISTS（已存在则 1050 跳过）。
      - last_month 已非空则跳过 INSERT，避免重复造数。
...
(26, '220426', '冯华', '审核', 8000.00, 0.48, 4800.00, 500.00, 13300.00)"""
    ...
    # ---- 建当月工资表（空表，待 Dify 工作流插入）----
    create_this = """\
CREATE TABLE IF NOT EXISTS `employee_salary_this_month` (
  `no` INT NOT NULL AUTO_INCREMENT COMMENT '序号',
  `employee_id` VARCHAR(10) NOT NULL COMMENT '员工工号',
  `name` VARCHAR(50) NOT NULL COMMENT '员工姓名',
  `position` VARCHAR(50) NOT NULL COMMENT '职位',
  ...
  UNIQUE KEY `uk_id` (`employee_id`)
    ...
    with conn.cursor() as cur:
        # 检查 last_month 是否已有数据
        try:
            cur.execute("SELECT COUNT(*) c FROM employee_salary_last_month")
            last_has_data = cur.fetchone()["c"] > 0
        except Exception:
            last_has_data = False

        for sql in (create_last, create_this):
            try:
                cur.execute(sql)
                created += 1
            except pymysql.err.OperationalError as e:
                if e.args[0] == 1050:
                    ...
                    print(f"[salary] 建表异常：{e}")

        # 灌入上月数据（仅当表为空时）
        if not last_has_data:
            try:
                cur.execute(insert_last)
                inserted = cur.rowcount
            except pymysql.err.OperationalError as e:
                if e.args[0] == 1062:  # Duplicate entry（UNIQUE uk_id）
                    inserted = 0
                else:
                    print(f"[salary] INSERT 异常：{e}")
        else:
            print("[salary] employee_salary_last_month 已有数据，跳过 INSERT")
...
# 说明：employees 为空时插入 26 名真实花名册员工（工号 220401~220426，密码统一 123456）；
#       其余表按现有员工生成考勤/薪资/报销/考核查询日志/考核成绩演示数据。全部幂等，重复跑不重复造。
# ------------------------------------------------------------
# 真实花名册（name, department, position, permissions 字符串）。仅在 employees 为空时插入。
# 与 migrate_real_roster.py 保持同源；permissions 用「、」分隔，插入时切分为 JSON 数组。
DEMO_ROSTER = [
    ("冯霞",   "总经办", "总经理",     "总经办、商务部、运营部、车队、财务部"),
    ("胡倩",   "总经办", "总经理助理", "总经办、商务部、运营部、车队、财务部"),
    ("黄秀英", "商务部", "商务总监",   "商务总监、营销经理、营销专员"),
    ("赵秀兰", "商务部", "营销经理",   "营销经理、营销专员"),
    ("谢洋",   "商务部", "营销经理",   "营销经理、营销专员"),
    ("许梦",   "商务部", "营销专员",   "营销专员"),
    ("李思",   "商务部", "营销专员",   "营销专员"),
    ("郑磊",   "运营部", "运营总监",   "运营总监、调度经理、调度专员"),
    ("许磊",   "运营部", "调度经理",   "调度经理、调度专员"),
    ("高秀兰", "运营部", "调度专员",   "调度专员"),
    ("李洋",   "运营部", "调度专员",   "调度专员"),
    ("何玲",   "运营部", "运营经理",   "运营经理、运营专员"),
    ("胡娟",   "运营部", "运营专员",   "运营专员"),
    ("袁芬",   "运营部", "运营专员",   "运营专员"),
    ("杨思",   "运营部", "客服经理",   "客服经理、客服专员"),
    ("董秀英", "运营部", "客服专员",   "客服专员"),
    ("傅秀英", "运营部", "客服专员",   "客服专员"),
    ("杨梦",   "车队",   "总车队长",   "总车队长、车队长、车队文员"),
    ("梁怡",   "车队",   "车队长",     "车队长、车队文员"),
    ("林飞",   "车队",   "车队长",     "车队长、车队文员"),
    ("萧平",   "车队",   "车队文员",   "车队文员"),
    ("陈平",   "车队",   "车队文员",   "车队文员"),
    ("邓玲",   "财务部", "财务总监",   "财务总监、会计、出纳、审核"),
    ("罗强",   "财务部", "会计",       "会计"),
    ("林伟",   "财务部", "出纳",       "出纳"),
    ("冯华",   "财务部", "审核",       "审核"),
]
...
def gen_demo_dataset(conn):
    ...
    with conn.cursor() as cur:
        ...
        if cur.fetchone()["c"] == 0:
            ...
            for i, (name, dept, pos, perms_str) in enumerate(DEMO_ROSTER):
                perms = json.dumps([p.strip() for p in perms_str.split("、") if p.strip()],
                                   ensure_ascii=False)
                cur.execute(ins, (i + 1, 220401 + i, name, h, perms, pos, dept))
            conn.commit()
            print(f"[demo] 已插入 {len(DEMO_ROSTER)} 名真实花名册员工（工号 220401~220426，密码统一 123456）")
        else:
